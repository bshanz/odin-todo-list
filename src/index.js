...
const thisWeekCheck = () => {
  ...
  let taskDates = myTasks.forEach((task) => {
    if (isThisWeek(parseISO(task.date)) === true) {
      // TODO: use the task's render method.
    }
  });
};
...
const loadEachTask = () =>{
  Object.keys(localStorage).forEach(function(key){
    ...
    const task = new Task(loadedItem.title, loadedItem.description,
                          loadedItem.project, loadedItem.date);
    myTasks.push(task);
    const toDoCard = task.render();
    grid.appendChild(toDoCard);
 });
